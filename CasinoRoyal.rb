...
############Begin Russian Roulette Class
class RussianRoulette
  def spin
    rand(6)
  end
  def gun
    chamber = ["click", "click", "click", "click", "click", "click",]
    chamber[spin] = "BANG!"
    chamber
  end
  def players
    player = ['Bob', 'Joe', 'Carly', 'Sam', 'Jim', 'Kammy']
  end
end
############End Russian roulette class
...
loop do
  ...
  puts "What game would you like to play? Options: Slots, BlackJack, Roulette"
  input = gets.chomp!

  ##########Start Russian Roulette loop
  if input.downcase == "roulette"
    loop do
      puts "What is your name?"
      name = gets.chomp
      gun = RussianRoulette.new
      @player = gun.players
      @player[gun.spin - 1] = name
      
      loop do
        if @player.include?(name) == false
          break
        end
        barrel = gun.gun
        i = -1
        ...
        if (i > @player.length)
          i = i - @player.length
          puts "#{@player[i]} is dead."
          @player.delete(@player[i])
        elsif @player.length == 0
          puts "#{@player[0]} is dead."
          @player.delete(@player[0])
        else
          puts "#{@player[i]} is dead."
          @player.delete(@player[i])
        end
        if @player.include?(name) == false
          puts "You are dead.  Your money is now mine."
          wallet.sub(wallet.funds)
          puts "You have $#{wallet.funds}"
          break
        elsif (@player.length == 1) && (@player.include?(name))
          puts "You win!" 
        end
        puts "Players left in the game: #{@player}"
...
############end russian roulette loop

############start slot loop
elsif input.downcase == "slots"
  loop do
    wallet.sub(5)
    player = Slots.new
